...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = client.audio.transcribe_raw(model=args.model,
        file=file_reader,
        filename=args.file,
        # Optional
        response_format=args.response_format,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = client.audio.translate_raw(model=args.model,
        file=file_reader,
        filename=args.file,
        # Optional
        response_format=args.response_format,
...
class FineTuningJob:
    ...
    def list(cls, args):
        has_ft_jobs = False
        for fine_tune_job in client.fine_tuning.auto_paging_iter():
            has_ft_jobs = True
            print(fine_tune_job)
        if not has_ft_jobs:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = client.files.find_matching_files(name=user_provided_file or f.name,
            bytes=bytes,
            purpose="fine-tune")
            if len(matching_files) > 0:
                file_ids = [f["id"] for f in matching_files]
        ...
        sys.stdout.flush()
        status = client.files.wait_for_processing(resp["id"])
        if status != "processed":
            raise openai.error.OpenAIError(
    ...
    def create(cls, args):
        ...
        resp = client.fine_tuning.create(**create_args)
        print(resp)
        return

    @classmethod
    def get(cls, args):
        resp = client.fine_tuning.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = client.fine_tuning.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise openai.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = client.fine_tuning.retrieve(id=args.id)["result_files"][0]
        resp = client.files.download(id=result_file)
        print(resp.decode("utf-8"))
    ...
    def events(cls, args):
        ...
        while has_more:
            resp = client.fine_tuning.list_events(id=args.id, after=after)  # type: ignore
    ...
    @classmethod
    def cancel(cls, args):
        resp = client.fine_tuning.cancel(id=args.id)
        print(resp)
